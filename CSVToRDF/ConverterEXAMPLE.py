...
def add_category_type(example_uri, example_type):
    ...
    if not example_type or example_type.strip() == "":
        ...
        g.add((example_uri, RDF.type, onto_ns.GenericExampleType))
...
def process_exampleone_csv(csv_file_path):
    ...
    with open(csv_file_path, "r", encoding='utf-8') as csvfile:
        ...
        for row_index, row in enumerate(csv_reader, start=1):
            ...
            g.add((exampleone_uri, RDF.type, onto_ns.Thingone))
...
def process_exampletwo_csv(csv_file_path):
    ...
    with open(csv_file_path, "r", encoding='utf-8') as csvfile:
        ...
        for row_index, row in enumerate(csv_reader, start=1):
            ...
            head_id = safe_get_value(row, headers, "ExampleOneID")
            date_raw = safe_get_value(row, headers, "Date", clean_spaces=False)

            exampletwo_number = normalize_id(exampletwo_id)
            exampletwo_uri = create_uri("Thing2", exampletwo_number, default_ns)
            exampletwoone_uri = create_uri("Thing2-1", f"{head_id}_{row_index}", default_ns)
            g.add((exampletwo_uri, RDF.type, onto_ns.Thingtwo))
            g.add((exampletwo_uri, RDFS.label, Literal(exampletwo_number, datatype=XSD.string)))
            g.add((exampletwoone_uri, RDF.type, onto_ns.Thingtwoone))
            g.add((exampletwoone_uri, RDFS.label, Literal(f"{head_id}_{row_index}", datatype=XSD.string)))
            g.add((exampletwo_uri, onto_ns.relatedThingtwoone, exampletwoone_uri))
            date_raw = to_xsd_date(date_raw)
            if date_raw:
...
def process_examplethree_csv(csv_file_path):
    ...
    with open(csv_file_path, "r", encoding='utf-8') as csvfile:
        ...
        for row_index, row in enumerate(csv_reader, start=1):
            ...
            examplethree_uri = create_uri("Thing3", examplethree_number, default_ns)
            examplethreeone_uri = create_uri("Thing4", f"{head_id}_{row_index}", default_ns)
            g.add((examplethree_uri, RDF.type, onto_ns.Thingthree))
            g.add((examplethree_uri, RDFS.label, Literal(examplethree_number, datatype=XSD.string)))
            g.add((examplethree_uri, onto_ns.relatedThingfour, examplethreeone_uri))
            g.add((examplethreeone_uri, RDF.type, onto_ns.Thingfour))
            g.add((examplethreeone_uri, RDFS.label, Literal(f"{head_id}_{row_index}", datatype=XSD.string)))
...
def main():
    ...
    # Save RDF output
    output_file = "Output/example_rdf_Core.ttl"
